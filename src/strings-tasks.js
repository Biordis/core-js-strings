...
function removeLastOccurrences(str, value) {
  const proverka = str.lastIndexOf(value);
  if (proverka !== -1) {
    return str.slice(0, proverka) + str.slice(proverka + value.length);
  }
  return str;
}
...
function countVowels(str) {
  return str.match(/[aeiouyAEIOUY]/gi) === null
    ? 0
...
function isPalindrome(str) {
  const clean = str.toLowerCase().replace(/[^a-z0-9]/gi, '');
...
function findLongestWord(sentence) {
  const splitty = sentence.split(' ');
  let longestWord = '';
  for (let i = 0; i < splitty.length; i += 1) {
    const word = splitty[i];
...
function extractNameFromTemplate(value) {
  ...
  return `${mass[1]} ${mass[2].toString().replace('!', '')}`;
}
...
function encodeToRot13(str) {
  return str.replace(/[A-Za-z]/g, (x) => {
    const charCode = x.charCodeAt(0) + 13;
    return String.fromCharCode(
      (x <= 'Z' ? 90 : 122) >= charCode ? charCode : charCode - 26
    );
  });
}
